"use client";
import {
  Avatar,
  cn,
  Select,
  SelectItem,
  Tabs,
  Tab,
  type DateValue,
  type SharedSelection,
} from "@heroui/react";
import { Icon } from "@iconify/react";
import { useMemo } from "react"; // No es necesario 'use' ni 'useState' aquí si no se gestiona estado local

import { DurationEnum, durations, type TimeSlot, timeZoneOptions } from "./calendar";
import { format } from "date-fns"; // Importar format de date-fns
import { enUS, es } from "date-fns/locale"; // Importar los locales necesarios

// Mapa de locales para date-fns
const dateFnsLocales = {
  'en': enUS,
  'es': es,
};

interface BookingDetailsProps {
  className?: string;
  selectedTimeZone: string;
  selectedDuration: DurationEnum;
  onTimeZoneChange?: (keys: SharedSelection) => void;
  onDurationChange?: (selectedKey: React.Key) => void;
  selectedTimeSlotRange?: TimeSlot[];
  selectedDate?: DateValue;
  pacienteData: { // Renombrado de 'Paciente' a 'pacienteData' para seguir convenciones de camelCase
    res: {
      paciente: string;
      hospital: string;
      procedimiento: string;
      diasEspera: number;
    }
  };
  currentLangCode: 'en' | 'es'; // Añadir prop para el código de idioma actual
}

export default function BookingDetails({
  className,
  selectedTimeZone,
  selectedDuration,
  onDurationChange,
  onTimeZoneChange,
  selectedTimeSlotRange,
  selectedDate,
  pacienteData, // Usar el nombre de prop actualizado
  currentLangCode, // Recibir la prop del idioma
}: BookingDetailsProps) {
  // console.log("BookingDetails pacienteData:", pacienteData);
  const bookingDate = useMemo(() => {
    if (selectedDate) {
      // Convertir DateValue a un objeto Date nativo para date-fns
      const dateObj = selectedDate.toDate(selectedTimeZone); // Usar la zona horaria seleccionada
      // Formatear la fecha usando el locale actual
      return format(dateObj, "EEEE, MMMM d, yyyy", { locale: dateFnsLocales[currentLangCode] });
    }
    return "";
  }, [selectedDate, selectedTimeZone, currentLangCode]); // Añadir currentLangCode a las dependencias

  return (
    <div className={cn("flex flex-col p-6 lg:w-[220px] lg:px-4 lg:pt-8", className)}>
      <Avatar
        className="mb-3 shadow-md"
        size="sm"
        src="https://i.pravatar.cc/150?u=a042581f4e29026704k"
      />
      {/* {JSON.stringify(pacienteData?.res)} */}
      {/* Mostrar el nombre del paciente si está disponible */}
      {pacienteData?.res.paciente && (
        <p className="text-xs font-medium text-default-700">Paciente: <b>{pacienteData.res.paciente}</b></p>
      )}
      <p className="text-xs font-medium text-default-700">Hospital: <b>{pacienteData.res.hospital}</b></p>
      <p className="text-xs font-medium text-default-700">Procedimiento: <b>{pacienteData.res.procedimiento}</b></p>
      <p className="text-xs font-medium text-default-700">Días de espera: <b>{pacienteData.res.diasEspera}</b></p>
      {/* <p className="mb-4 text-small text-default-500">
        Lorem ipsum dolor sit amet cons ectetur. Turpis gravida eget felis senectus eleifend.
      </p> */}
      <div className="mb-6 flex flex-col gap-3">
        &nbsp;
        <div
          className={cn("flex items-start gap-2", {
            hidden: !bookingDate,
          })}
        >
          <Icon className="text-default-300" icon="solar:calendar-minimalistic-bold" width={16} />
          <div className="text-xs font-medium text-default-600">
            <p>{bookingDate}</p>
            {/* Mostrar el rango de tiempo si está disponible y tiene al menos dos elementos */}
            {selectedTimeSlotRange && selectedTimeSlotRange.length >= 2 && (
              <p>{`${selectedTimeSlotRange[0].label} - ${selectedTimeSlotRange[1].label}`}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Icon className="text-default-300" icon="solar:clock-circle-bold" width={16} />
          <p className="text-xs font-medium text-default-600">{selectedDuration}</p>
        </div>
        <div className="flex items-center gap-2">
          <Icon className="text-default-300" icon="solar:videocamera-bold" width={16} />
          <p className="text-xs font-medium text-default-600">Zoom</p>
        </div>
        <div className="flex items-center">
          <Icon className="flex-shrink-0 text-default-300" icon="solar:global-linear" width={16} />
          <Select
            aria-label="Time Zone"
            className="max-w-48"
            classNames={{
              trigger: "h-4 min-h-4 bg-transparent border-none shadow-none",
              value: "text-xs font-medium group-data-[has-value=true]:text-default-600",
            }}
            items={timeZoneOptions}
            placeholder="Select time zone"
            selectedKeys={[selectedTimeZone]}
            size="sm"
            variant="faded"
            onSelectionChange={onTimeZoneChange}
          >
            {(option) => (
              <SelectItem
                key={option.value}
                className="capitalize"
                classNames={{
                  title: "text-xs font-medium text-default-600",
                  description: "text-xs font-medium text-default-600",
                }}
              >
                {option.label}
              </SelectItem>
            )}
          </Select>
        </div>
      </div>
      <Tabs classNames={{ tab: "h-6" }} size="sm" onSelectionChange={onDurationChange}>
        {durations.map((duration) => (
          <Tab key={duration.key} title={duration.label} />
        ))}
      </Tabs>
    </div>
  );
}
